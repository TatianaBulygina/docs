...
В каталогах размещаются ресурсы — виртуальные машины, диски и др. При создании ресурса необходимо указать, в каком каталоге он будет создан. Создать каталог внутри каталога в настоящий момент невозможно.
...
Участник облака — это пользователь, которому назначена роль `{{ roles-cloud-member }}` на это облако. Данная роль автоматически назначается пользователю при его добавлении в облако.

Для выполнения операций с каким либо ресурсом в облаке необходимо, чтобы пользователь имел аккаунт на Яндаксе, и чтобы ему была назначена роль участника облака. Исключение — ресурсы с [публичным доступом](#public-access).

Сама роль `{{ roles-cloud-member }}` не дает прав на выполнение каких-либо операций с ресурсами. Эта роль используется в сочетании с другими ролями, например {{ roles-admin }}, {{ roles-editor }} и {{ roles-viewer }}. 
...
Как и каталоги в файловой системе, каталоги в Яндекс.Облаке упрощают управление ресурсами и помогают их систематизировать. Вы можете группировать ресурсы в каталоги по типу ресурса, по проекту, по отделу, который работает с этими ресурсами, или по любому другому признаку.
...
Это значит, что права на облако распространяются на все каталоги и ресурсы внутри облака. Права на каталог распространяются на все ресурсы в каталоге. 

Некоторые типы ресурсов могут созданы не в каталогах, поэтому они имеют собственную логику проверки прав доступа. Примером такого ресурса сожет являться сервисный аккаунт. При управлении ключами доступа для сервисного аккаунта проверяются права доступа на этот сервисный аккаунт.
